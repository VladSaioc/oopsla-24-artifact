...
Because the dataset used for our evaluation consists of proprietary code, we cannot directly disclose it. To compensate we supply the intermediate representation our tool manipulate (i.e. Promela models) derived from the original program fragments where variable names have been anonymized so as not to disclose any information about the business logic.

The Promela models support our claims about the proliferation of parametric concurrent program fragments, a subset of which may be modeled in VirGo. In order the claims are:
...
* Aggregate metrics over the verification time for individual fragments and the number of verifications carried out for each strategy.
...
In this section, we quickly set up Ginger, run it on a sample set of input fragments, and inspect the results. The input sample consists of the first 9 program fragments as they appear in our complete benchmark.
...
```
   ...
   The script above will start the Docker container, run Ginger on the sample inputs, and start a session within the container.

2) You can gather results of running on the fragment sample with:
```shell
# Rooted at /usr/app
collect-metrics.sh "./fragments-sample"
```

3) The output should display the following:
```shell
Parametric fragments discovered (Table 1 - Column 2)
Channel :: found 3 matches
WaitGroup: :: found 3 matches
Both :: found 3 matches
Total :: found 9 matches

Expressible fragments (Table 1 - Column 3)
Channel :: found 1 matches
...
Total :: found 3 matches

Efficacy of Strat. 3 (Table 1 - Column 7)
Channel :: found 0 matches
WaitGroup :: found 0 matches
Both :: found 0 matches
Total :: found 0 matches

Verification errors :: found 0 matches

Verification time (Table 2).
Strat. 1 :: Avg: 1767ms; P50: 1493ms; P90: 2743ms; Max: 2743ms
Performed: 4
Strat. 2 :: Avg: 2871ms; P50: 2894ms; P90: 3056ms; Max: 3056ms
Performed: 3
Strat. 3 :: Avg: 0ms; P50: 0ms; P90: 0ms; Max: 0ms
Performed: 0

Complexity metrics
FV occurrences  ::  Min: 2; Avg: 2.25; P50: 2; P90: 3; Max: 3
Unique FVs  ::  Min: 1; Avg: 1; P50: 1; P90: 1; Max: 1
```
...
cat ./fragments-sample/dir12/dir13/loc-6-ginger.pml-results.res
# OR
vi ./fragments-sample/dir12/dir13/loc-6-ginger.pml-results.res
```
...
To reproduce the benchmark experiment, run:
```shell
./run-benchmark.sh
```
This will spin a Docker container that runs Ginger on all program fragments in the benchmark.

The build script will also start a session within the Docker container at `/usr/app`. To gather the results of the analysis on all fragments, run the following:
```shell
# Rooted at /usr/app
collect-metrics.sh fragments
```

<!-- TODO: Remove in a later version of the artifact -->
**NOTE:** Due to a bugfix in the Promela translation front-end of Ginger, there are slight differences between the results presented in the submitted version of the paper and those reported by the benchmark. Overall, two fragments are no longer parametric but more fragments are supported by Strategies 1 and 2; the conclusions of our evaluation remain the same. We will update the paper accordingly.

The output of the command above are as follows:
```shell
Parametric fragments discovered (Table 1 - Column 2)
Channel :: found 258 matches
WaitGroup: :: found 205 matches
Both :: found 120 matches
Total :: found 583 matches

Expressible fragments (Table 1 - Column 3)
Channel :: found 159 matches
WaitGroup :: found 171 matches
Both :: found 48 matches
Total :: found 378 matches

Efficacy of verification back-end (Table 1 - Column 4)
Channel :: found 157 matches
WaitGroup :: found 170 matches
Both :: found 46 matches
Total :: found 373 matches

Efficacy of Strat. 1 (Table 1 - Column 5)
Channel :: found 41 matches
WaitGroup :: found 86 matches
Both :: found 18 matches
Total :: found 145 matches

Efficacy of Strat. 2 (Table 1 - Column 6)
Channel :: found 116 matches
WaitGroup :: found 67 matches
Both :: found 25 matches
Total :: found 208 matches

Efficacy of Strat. 3 (Table 1 - Column 7)
Channel :: found 0 matches
WaitGroup :: found 17 matches
Both :: found 3 matches
Total :: found 20 matches

Verification errors :: found 5 matches

Verification time (Table 2)
Strat. 1 :: Avg: 1826.95ms; P50: 1360ms; P90: 2606ms; Max: 4872ms
Performed: 378
Strat. 2 :: Avg: 2454.36ms; P50: 2513ms; P90: 2733ms; Max: 5385ms
Performed: 233
Strat. 3 :: Avg: 2400.32ms; P50: 2481ms; P90: 2661ms; Max: 5756ms
Performed: 25

Complexity metrics
FV occurrences  ::  Min: 1; Avg: 2.2; P50: 2; P90: 3; Max: 6
Unique FVs  ::  Min: 1; Avg: 1.08; P50: 1; P90: 1; Max: 3
```
...
* A brief comparison with state of the art on tools on selected examples can be found [here](./appendix.md).
