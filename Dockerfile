...
WORKDIR /usr/app
COPY ./fragments /usr/app/fragments
COPY ./fragments-sample /usr/app/fragments-sample
...
WORKDIR /usr/app/ginger
RUN bash ./recursive-run.sh "/usr/app/fragments-sample"

# Run the result parser
WORKDIR /usr/app/result-parser
RUN bash ./collect-metrics.sh "/usr/app/fragments-sample"
...
# Run the benchmark on all fragments
WORKDIR /usr/app/ginger
RUN bash ./recursive-run.sh "/usr/app/fragments"

# Run the result parser on all fragments
WORKDIR /usr/app/result-parser
RUN bash ./collect-metrics.sh "/usr/app/fragments"
